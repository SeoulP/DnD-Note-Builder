...
public partial class ItemDetailPane : ScrollContainer
{
    ...
    {
        if (_item == null || _aliasChipsRow == null) return;
        ...
        foreach (var alias in _db.EntityAliases.GetForEntity("item", _item.Id))
        {
        ...
        addInput.TextSubmitted += text =>
        {
            ...
            _db.EntityAliases.Add(new DndBuilder.Core.Models.EntityAlias { CampaignId = _item.CampaignId, EntityType = "item", EntityId = _item.Id, Alias = t });
            LoadAliases();
        };
