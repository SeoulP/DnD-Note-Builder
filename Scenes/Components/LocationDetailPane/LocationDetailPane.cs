...
public partial class LocationDetailPane : ScrollContainer
{
    ...
    {
        if (_location == null || _aliasChipsRow == null) return;
        ...
        foreach (var alias in _db.EntityAliases.GetForEntity("location", _location.Id))
        {
        ...
        addInput.TextSubmitted += text =>
        {
            ...
            _db.EntityAliases.Add(new DndBuilder.Core.Models.EntityAlias { CampaignId = _location.CampaignId, EntityType = "location", EntityId = _location.Id, Alias = t });
            LoadAliases();
        };
