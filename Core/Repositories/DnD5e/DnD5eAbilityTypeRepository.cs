...
{
    public class DnD5eAbilityTypeRepository
    {
        private readonly SqliteConnection _conn;

        public DnD5eAbilityTypeRepository(SqliteConnection conn) => _conn = conn;
        ...
        public List<DnD5eAbilityType> GetAll(int campaignId)
        {
            var list = new List<DnD5eAbilityType>();
            var cmd  = _conn.CreateCommand();
            ...
            while (reader.Read())
                list.Add(new DnD5eAbilityType { Id = reader.GetInt32(0), CampaignId = reader.GetInt32(1), Name = reader.GetString(2) });
            return list;
        }

        public int Add(DnD5eAbilityType type)
        {
            var cmd = _conn.CreateCommand();
