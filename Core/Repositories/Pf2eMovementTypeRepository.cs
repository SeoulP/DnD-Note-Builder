...
namespace DndBuilder.Core.Repositories
{
    public class Pf2eMovementTypeRepository
    {
        ...
        {
            "Land", "Fly", "Swim", "Climb", "Burrow", "Troop Movement",
        };
        ...
        public void SeedDefaults(int campaignId)
        {
            ...
            {
                var cmd = _conn.CreateCommand();
                cmd.CommandText = @"INSERT INTO pathfinder_movement_types (campaign_id, name)
                    SELECT @cid, @name WHERE NOT EXISTS
                        (SELECT 1 FROM pathfinder_movement_types WHERE campaign_id = @cid AND name = @name)";
